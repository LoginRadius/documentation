...
- [**One Touch Login by Email**](#one-touch-login-by-email): The customer can log in directly by clicking on the link received in the Email. This email does not have to be pre-registered on the web/mobile application. Thus, the login flow will always remain the same whether you are already a customer of a web/mobile application or not.

- [**One Touch Login by SMS**](#one-touch-login-by-sms): The customer can log into a web/mobile application directly entering the OTP received on their phone number.

> **Note**: One Touch Login SMS will work only if the [**Phone Login**](https://www.loginradius.com/docs/authentication/tutorial/phone-login/) feature is enabled.
...
Login to your [**Admin Console**](https://adminconsole.loginradius.com/) account and navigate to [**Authentication configuration > Advance Login Methods > One Touch Login Settings**](https://adminconsole.loginradius.com/platform-configuration/authentication-configuration/advance-login-methods/one-touch-login-settings).

The following screen appears:

![one touch enable](https://apidocs.lrcontent.com/images/image-38_2121980436687a701b6d1a7.94957080.png "one touch enable")

The above screen displays that the One Touch Login is enabled for your account since the One Touch Login box is in green with the enabled text. If not **enabled** for your account, raise a request to the [ LoginRadius Support Team](https://adminconsole.loginradius.com/support/tickets/open-a-new-ticket)

> **Note:** In addition to this feature if you also want to disable the use of **Login APIs**, where password is **required field**, such as [**Auth Login By Email**](https://www.loginradius.com/docs/api/v2/customer-identity-api/authentication/auth-login-by-email/), [**Auth Login By Username**](https://www.loginradius.com/docs/api/v2/customer-identity-api/authentication/auth-login-by-username/) or [**Auth Login By Phone**](https://www.loginradius.com/docs/api/v2/customer-identity-api/phone-authentication/phone-login/), and etc., please raise a request to the [ LoginRadius Support Team](https://adminconsole.loginradius.com/support/tickets/open-a-new-ticket).
...
- [One Touch Login by Email](#one-touch-login-by-email)
- [One Touch Login by SMS](#one-touch-login-by-sms)
...
**Step 1:** To manage the email content and settings for the One Touch Login by Email, navigate to [**Platform Configuration >Authentication configuration> Advance Login Methods > One Touch Login Email**](https://adminconsole.loginradius.com/platform-configuration/authentication-configuration/advance-login-methods/one-touch-login-email)
 
The following screen appears:

![template](https://apidocs.lrcontent.com/images/image-39_8647230326687aa9a3785b6.22460222.png "template")

**Step 2:** Expand the **One Touch Login Email Settings** panel. Enter the desired [**Request Limit**](https://www.loginradius.com/docs/authentication/concepts/email-communications/#partglobalemailsettings5), [**Request disabled Period**](https://www.loginradius.com/docs/authentication/concepts/email-communications/#partglobalemailsettings5) and [**One touch Login  Email Token Validity Limit**](https://www.loginradius.com/docs/authentication/concepts/email-communications/#partglobalemailsettings5) as highlighted in the following screen:
...
You can perform actions such as [**Add Template**](https://www.loginradius.com/docs/authentication/concepts/email-communications/#partemailtemplates6), [**Edit Template**](https://www.loginradius.com/docs/authentication/concepts/email-communications/#partemailtemplates6), [**Send Test Email**](https://www.loginradius.com/docs/authentication/concepts/email-communications/#partemailtemplates6) and [**Delete Template**](https://www.loginradius.com/docs/authentication/concepts/email-communications/#partemailtemplates6).
...
**Step 1:** To manage the SMS content and settings for  the **One Touch Login by SMS**, navigate to [**Platform Configuration -> Authentication Configuration -> Advanced Login Methods -> One Touch Login SMS**](https://adminconsole.loginradius.com/platform-configuration/authentication-configuration/advance-login-methods/one-touch-login-sms)

The following screen appears: 

![sms](https://apidocs.lrcontent.com/images/unnamed-23_13713026006687a941538c44.47453721.png "sms")

**Step 2**: Expand the **One Touch Login SMS Settings** panel. Enter the desired [**Request Limit**](https://www.loginradius.com/docs/authentication/concepts/sms-communication/#partsmstemplatesettings1), [**Request disabled Period**](https://www.loginradius.com/docs/authentication/concepts/sms-communication/#partsmstemplatesettings1) and [**One touch Login OTP Validity Limit**](https://www.loginradius.com/docs/authentication/concepts/sms-communication/#partsmstemplatesettings1) as highlighted in the following screen:
...
You can perform actions such as [**Add**](https://www.loginradius.com/docs/authentication/concepts/sms-communication/#addtemplate4), [**Edit**](https://www.loginradius.com/docs/authentication/concepts/sms-communication/#edittemplate3), [**Reset**](https://www.loginradius.com/docs/authentication/concepts/sms-communication/#resettemplate6) and [**Delete**](https://www.loginradius.com/docs/authentication/concepts/sms-communication/#deletetemplate5) template. 
...
- [**Identity Experience Framework**](#idx-deployment): You should refer to these deployment steps if you are targeting LoginRadius Identity Platform implementation using IDX. 

- [**JavaScript Libraries**](#javascript-deployment): You should refer to these deployment steps if you are targeting LoginRadius Identity Platform implementation using JavaScript.
...
>**Note:**
> - To implement the One Touch Login using API refer to this [document](https://www.loginradius.com/docs/api/v2/customer-identity-api/one-touch-login/one-touch-login-overview/)
> - To implement the One Touch Login using technology-specific SDK refer to this [document](https://www.loginradius.com/docs/api/v2/deployment/sdk-libraries/overview/)
...
1. Navigate to [**Deployment > Identity Experience Framework (Hosted)**](https://adminconsole.loginradius.com/deployment/idx) and the following screen appears:
...
6. Navigate to your Auth page URL (`https://<sitename>.hub.loginradius.com/auth.aspx`) , where [sitename](https://www.loginradius.com/docs/api/v2/admin-console/deployment/get-site-app-name/) is the name of your LoginRadius Site. 
...
> We have additional language specific examples [here](https://www.loginradius.com/docs/api/v2/deployment/identity-experience-framework/hosted/usage/#tokenhandling1) if you want to capture this token in other programming languages.
...
[Passwordless Login](https://www.loginradius.com/docs/authentication/tutorial/passwordless-login#partnextsteps3)

[Smart Login](https://www.loginradius.com/docs/authentication/tutorial/smart-login/)

[JS Customizations](https://www.loginradius.com/docs/api/v2/deployment/js-libraries/getting-started/#login7)